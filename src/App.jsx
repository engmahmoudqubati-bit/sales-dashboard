...
import LocationMonthChart from "./components/LocationMonthChart"
...
export default function App() {
  ...
  const [filters, setFilters] = useState({ branch:"All", month:"All" })

  const t   = T[lang]
  const dir = lang==="ar" ? "rtl" : "ltr"

  useEffect(() => {
    fetch("https://sales-api-production-da66.up.railway.app/api/sales?limit=100000&offset=0")
      .then(res => res.json())
      .then(apiData => {
  ...
  if(loading) return (
    <div style={{ display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", height:"100vh", background:"#0f172a", gap:"16px" }}>
      <div style={{ width:"40px", height:"40px", border:"3px solid #334155", borderTop:"3px solid #38bdf8", borderRadius:"50%", animation:"spin 1s linear infinite" }} />
      <p style={{ fontSize:"16px", color:"#94a3b8" }}>Loading dashboard...</p>
    ...
    <div style={{ padding:"2rem", maxWidth:"1400px", margin:"0 auto", color:"var(--text)", direction:dir }}>
      ...
      {data.length>0&&(
        <>
          ...
          <LocationMonthChart data={filtered} cardStyle={cardStyle} dark={dark} t={t} dir={dir} />
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr", gap:"20px", marginTop:"1.5rem" }}>
            <PieChartBranch  data={filtered} cardStyle={cardStyle} t={t} />
          </div>
          <AIAgent data={filtered} cardStyle={cardStyle} dark={dark} t={t} filters={filters} />
        </>
      )}
    </div>
  )
